...
class ControllerStep2 extends Controller {
	...
	private function validate() {
		if (phpversion() < '5.0') {
			$this->error['warning'] = '警告: 运作网站需要 PHP5 以上!';
		}
		...
		if (ini_get('session.auto_start')) {
			$this->error['warning'] = '警告: 运作网站需要设置 session.auto_start 为 enabled!';
		}

		if (!extension_loaded('mysql')) {
			$this->error['warning'] = '警告: 需要安装MYSQL的扩展才能运行网站!';
		}

		if (!extension_loaded('gd')) {
			$this->error['warning'] = '警告: 需要安装GD的扩展,才能运行网站!';
		}

		if (!extension_loaded('zlib')) {
			$this->error['warning'] = '警告: 需要安装ZLIB的扩展,才能运行网站!';
		}
